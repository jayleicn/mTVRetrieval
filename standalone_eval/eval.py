    ...
    if use_desc_type:
        metrics_by_type["desc_type_ratio"] = "v {} t {} vt {}"\
            .format(*[get_rounded_percentage(1.0 * np.sum(desc_types == desc_type2idx[k]) / len(desc_types))
                      for k in ["v", "t", "vt"]])
    return metrics, metrics_by_type, pred_info_matrix_collection
...
def eval_retrieval(submission, ground_truth, iou_thds=(0.5, 0.7), verbose=True, match_number=True, use_desc_type=True):
    ...
    for task_type in submitted_task_types:
        metrics, metrics_by_type, corrects = eval_by_task_type(
        ...
        corrcets_dict[task_type] = corrects
    ...
    return eval_metrics  # , corrcets_dict
...
def eval_main():
    ...
    # results, corrcets_dict = eval_retrieval(submission, gt, iou_thds=(0.5, 0.7), verbose=verbose)
    results = eval_retrieval(submission, gt, iou_thds=(0.5, 0.7), verbose=verbose)
    if verbose:
        print(json.dumps(results, indent=4))

    # save_corrects_path = args.save_path.replace(".json", ".npz")
    # np.savez(save_corrects_path, **corrcets_dict)
