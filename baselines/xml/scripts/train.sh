...
# Usage:
# bash baselines/xml/scripts/train.sh zh video_sub resnet_i3d ANY_OTHER_PYTHON_ARGS
...
results_root=baselines/xml/results
vid_feat_size=2048
exp_id=exp
extra_args=()
...
case ${dset_name} in
    tvr)
        ...
        extra_args+=(16)
#        /bert_feature_en/sub_query/tvr_query_pretrained_w_sub_query.h5
        desc_bert_path=${feature_root}/bert_feature_${lang}/sub_query/tvr_query_pretrained_w_sub_query.h5
        if [[ ${ctx_mode} == *"sub"* ]] || [[ ${ctx_mode} == "sub" ]]; then
...
# added no_cross_att to the original XML model.
python baselines/xml/train.py \
--lang ${lang} \
--dset_name=${dset_name} \
