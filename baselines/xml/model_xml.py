...
from baselines.xml.model_components import \
    BertAttention, LinearLayer, BertSelfAttention, TrainablePositionalEncoding, ConvEncoder
...
class XML(nn.Module):
    ...
    def forward(self, query_feat, query_mask, video_feat, video_mask, sub_feat, sub_mask,
                tef_feat, tef_mask, st_ed_indices, return_neighbor_embeddings=False):
        """
        Args:
            query_feat: (N, Lq, Dq)
            query_mask: (N, Lq)
        ...
        loss_dict = dict(
            ...
            loss_vr=float(loss_neg_ctx) + float(loss_neg_q),
            loss_overall=float(loss)
        )

        if return_neighbor_embeddings:
            neighbor_embeddings = dict(
                video_query=video_query if self.use_video else None,  # (N, D)
                sub_query=sub_query if self.use_sub else None,  # (N, D)
                # video_feat1=video_feat1,  # not needed?
                # video_feat2=video_feat2,
                sub1=self.pool_ctx_embeddings(sub_feat1, sub_mask) if self.use_sub else None,  # (N, D)
                sub2=self.pool_ctx_embeddings(sub_feat2, sub_mask) if self.use_sub else None,  # (N, D)
            )
            return loss, loss_dict, neighbor_embeddings
        else:
            return loss, loss_dict

    def pool_ctx_embeddings(self, ctx_embedding, ctx_mask, pool_type="mean"):
        """Simply take average of the embeddings
        Args:
            ctx_embedding: (N, L, D)
            ctx_mask: (N, L)
        ...
        """
        if pool_type == "max":
            pooled_ctx_embedding = torch.max(
                mask_logits(ctx_embedding, ctx_mask.unsqueeze(2)), dim=1
            )[0]  # (N, D)
        elif pool_type == "mean":
            ctx_embedding = ctx_embedding * ctx_mask.unsqueeze(2)  # (N, L, D)
            pooled_ctx_embedding = ctx_embedding.sum(1) / ctx_mask.sum(1, keepdims=True)  # (N, D)
        else:
            raise ValueError(f"Invalid pooling type: {pool_type}")
        return pooled_ctx_embedding
    ...
    def get_pred_from_raw_query(self, query_feat, query_mask,
        ...
        if return_modular_queries:
            return q2ctx_scores, st_prob, ed_prob, video_query, sub_query  # un-normalized masked probabilities!!!!!
        else:
...
def mask_logits(target, mask):
    """mask where 1 indicates valid bits and 0 indicates invalid."""
