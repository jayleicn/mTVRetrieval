...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        # self.parser.add_argument("--lang", type=str, choices=["en", "zh"])  # always has both languages
        ...
        self.parser.add_argument("--no_core_driver", action="store_true",
                                 help="hdf5 driver, default use `core` (load into RAM), if specified, use `None`"
                                      "use `pip install h5py==2.9.0', 3.1.0 would cause error.")
        ...
        self.parser.add_argument("--use_paired_datasets", action="store_true",
                                 help="use paired english-chinese inputs. "
                                      "Note that lw_neighbors > 0 will automatically turn this on")
        self.parser.add_argument("--lw_neighbors", type=float, default=0,
                                 help="weight of language neighbor constraints, "
                                      "a value > 0 will automatically use paired dataset across languages")
        self.parser.add_argument("--neighbors_names", type=str, default="query", nargs="+",
                                 choices=["query", "sub1", "sub2"],
                                 help="which embeddings to apply neighbor constraints to. "
                                      "query: --> video_query + sub_query;"
                                      "sub1: --> sub_feat1;"
                                      "sub2: --> sub_feat2.")
        ...
        self.parser.add_argument("--hard_negtiave_start_epoch", type=int, default=10,
        ...
        self.parser.add_argument("--harder_negtiave_start_epoch", type=int, default=20,
                                 help="which epoch to start hard negative sampling for video-level ranking loss,"
                                      "use -1 to disable")
        self.parser.add_argument("--harder_pool_size", type=int, default=10,
                                 help="hard negatives are still sampled, but from a harder pool.")
        self.parser.add_argument("--hardest_negtiave_start_epoch", type=int, default=30,
                                 help="which epoch to start hard negative sampling for video-level ranking loss,"
                                      "use -1 to disable")
        self.parser.add_argument("--hardest_pool_size", type=int, default=5,
                                 help="hard negatives are still sampled, but from a harder pool.")

        # sharing configs
        self.parser.add_argument("--share_vid_enc", action="store_true")
        self.parser.add_argument("--share_sub_enc", action="store_true")
        self.parser.add_argument("--share_q_enc", action="store_true")
        self.parser.add_argument("--share_pred", action="store_true")
        self.parser.add_argument("--share_q_mapping", action="store_true")
        self.parser.add_argument("--share_input_proj_in_shared_enc", action="store_true")
        ...
        self.parser.add_argument("--desc_bert_path_en", type=str, default=None)
        self.parser.add_argument("--sub_bert_path_en", type=str, default=None)
        self.parser.add_argument("--desc_bert_path_zh", type=str, default=None)
        self.parser.add_argument("--sub_bert_path_zh", type=str, default=None)
    ...
    def parse(self):
        ...
        if isinstance(self, TestOptions):
            ...
            # TODO temporary hack to avoid manually naming the variants
            sharing_related_attrs = [
                "share_vid_enc", "share_sub_enc", "share_q_enc",
                "share_pred", "share_q_mapping", "share_input_proj_in_shared_enc"
            ]
            variant_name = "share_" + "_".join([f"{e[6:13]}{int(getattr(opt, e))}" for e in sharing_related_attrs])
            if opt.lw_neighbors > 0:
                assert len(opt.neighbors_names) > 0, "Please specify at least a neighbor"
                variant_name += f"lw_neighbors{opt.lw_neighbors}_{'_'.join(opt.neighbors_names)}"

            opt.results_dir = os.path.join(opt.results_root,
                                           "-".join([opt.dset_name, opt.ctx_mode, variant_name, opt.exp_id,
        ...
        if hasattr(opt, "lw_neighbors") and opt.lw_neighbors > 0:
            opt.use_paired_datasets = True
